...
const rewards: readonly RewardDefinition[] = [
  {
    id: 'reward.lantern',
    ...
    label: 'Lanterne',
    description: 'Une petite lumière rejoint la clairière.',
  },
];
...
const chapters: readonly StoryChapter[] = [
  {
    ...
    worldId: 'world.firefly-forest',
    order: 1,
    title: 'La première lumière',
    body: 'Une luciole ouvre les yeux dans la clairière.',
    illustrationId: 'world.forest-stage-1',
    requiredCompletions: 1,
  },
];

describe('progression', () => {
  it('crée une réalisation et une récompense reliées une seule fois par leurs identifiants', () => {
    const completion = createCompletion(
      occurrence,
      'parent',
      'parent',
      'grant-1',
      { id: 'completion-1', createdAt: occurrence.completedAt! },
    );
    const grant = createRewardGrant(completion, rewards[0]!, {
      id: 'grant-1',
  ...
  it('reconstruit le monde depuis les récompenses sans dépendre d’une série', () => {
    ...
    const progress = rebuildWorldProgress({
      ...
      rewards,
      chapters,
      identity: { id: 'progress-1', createdAt: 'now' },
      celebrationAt: 'now',
    });

    expect(progress.completionCount).toBe(1);
    expect(progress.unlockedRewardIds).toEqual(['reward.lantern']);
    expect(progress.unlockedStoryChapterIds).toEqual(['chapter.1']);
  ...
  it('fait évoluer quatre états sans retirer les anciens objets', () => {
    ...
    expect(progress.completionCount).toBe(12);
    expect(progress.unlockedRewardIds).toEqual(['reward.lantern']);
  });
});
