...
function createProfile() {
  return createChildProfile(input, {
    id: 'child-1',
...
describe('ChildProfile', () => {
  it('crée un profil minimal sans date de naissance', () => {
  ...
  it('refuse un prénom vide ou excessivement long', () => {
    expectDomainError(
      () => createChildProfile({ ...input, displayName: '   ' }, { id: 'child-2', createdAt: 'now' }),
      'child.display-name-required',
    );
    expectDomainError(
      () =>
        createChildProfile(
          { ...input, displayName: 'x'.repeat(31) },
          { id: 'child-3', createdAt: 'now' },
        ),
  ...
  it('refuse une tranche d’âge inconnue même si une donnée importée contourne TypeScript', () => {
    expectDomainError(
      () =>
        createChildProfile(
          { ...input, ageBand: '11-13' as AgeBand },
          { id: 'child-4', createdAt: 'now' },
        ),
  ...
  it('met à jour le profil de manière immuable et révisionnée', () => {
    ...
    const updated = updateChildProfile(
      original,
      { displayName: 'Maddie l’exploratrice', readingLevel: 'short-text' },
      '2026-07-16T09:00:00.000Z',
    );

    expect(original.displayName).toBe('Maddie');
    expect(updated.displayName).toBe('Maddie l’exploratrice');
    expect(updated.readingLevel).toBe('short-text');
  ...
  it('archive de façon idempotente puis exige une restauration avant modification', () => {
    ...
    expectDomainError(
      () => updateChildProfile(archived, { displayName: 'Nouveau nom' }, 'later'),
