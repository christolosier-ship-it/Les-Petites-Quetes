...
function datesForSchedule(schedule: QuestSchedule, fromDate: string, toDate: string): readonly string[] {
  if (schedule.kind !== 'weekly') {
    return compareLocalDates(schedule.startDate, fromDate) >= 0 && compareLocalDates(schedule.startDate, toDate) <= 0 ? [schedule.startDate] : [];
  }
...
export function generateQuestOccurrences(input: GenerateOccurrencesInput): readonly QuestOccurrence[] {
  ...
  for (const childId of input.schedule.childIds) {
    ...
    const questTemplateId = input.resolveQuestVariantId(input.schedule.questFamilyId, child.ageBand, input.schedule.questTemplateId);
    for (const localDate of dates) {
      ...
      generated.push({
        ...
        questTemplateId,
        questFamilyId: input.schedule.questFamilyId,
        worldId: input.schedule.worldId,
        childId: child.id,
        localDate,
