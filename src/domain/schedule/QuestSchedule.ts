...
function normalizeChildIds(childIds: readonly string[]): readonly string[] {
  assertDomain(
    ...
    'Une planification doit concerner au moins un enfant.',
    'childIds',
  );
  const normalized = childIds.map((childId, index) =>
    requireIdentifier(
      childId,
      'schedule.identifier-required',
      'Chaque enfant doit posséder un identifiant.',
      `childIds.${index}`,
    ),
  );
  assertDomain(
    !hasDuplicates(normalized),
    'schedule.child-duplicated',
    'Un enfant ne peut être attribué deux fois à la même planification.',
    'childIds',
  );
  return normalized;
}
...
function normalizeWeekdays(
  kind: QuestScheduleInput['kind'],
  weekdays: readonly Weekday[] | undefined,
): readonly Weekday[] | undefined {
  if (kind !== 'weekly') {
    assertDomain(
      weekdays === undefined || weekdays.length === 0,
      'schedule.weekdays-not-allowed',
      'Les jours de semaine sont réservés aux planifications hebdomadaires.',
      'weekdays',
    );
    return undefined;
  }
  ...
  assertDomain(
    ...
    'Une planification hebdomadaire doit contenir au moins un jour.',
    'weekdays',
  );
  for (const weekday of weekdays) {
    assertDomain(
      belongsTo(weekday, WEEKDAYS),
      'schedule.invalid-weekday',
      'Un jour de semaine est inconnu.',
      'weekdays',
    );
  }
  assertDomain(
    ...
    'Un jour de semaine ne peut apparaître qu’une fois.',
    'weekdays',
  );
  return [...weekdays];
}
...
function normalizeInput(input: QuestScheduleInput): QuestScheduleInput {
  assertDomain(
    belongsTo(input.kind, SCHEDULE_KINDS),
    'schedule.invalid-kind',
    'Le type de planification est inconnu.',
    'kind',
  );
  assertDomain(
    belongsTo(input.dayMoment, DAY_MOMENTS),
    'schedule.invalid-day-moment',
    'Le moment de la journée est inconnu.',
    'dayMoment',
  );
  assertDomain(
    belongsTo(input.priority, QUEST_PRIORITIES),
    'schedule.invalid-priority',
    'La priorité de la quête est inconnue.',
    'priority',
  );
  assertDomain(
    belongsTo(input.validationMode, VALIDATION_MODES),
    'schedule.invalid-validation-mode',
    'Le mode de validation est inconnu.',
    'validationMode',
  );
  ...
  if (input.kind !== 'weekly') {
    assertDomain(
      ...
      'Une planification immédiate ou ponctuelle ne possède pas de date de fin.',
      'endDate',
    );
  ...
  return {
    questTemplateId: requireIdentifier(
      input.questTemplateId,
      'schedule.identifier-required',
      'Un modèle de quête est requis.',
      'questTemplateId',
    ),
...
function assertMutable(schedule: QuestSchedule): void {
  assertDomain(
    ...
    'Une planification supprimée ne peut plus être modifiée.',
  );
}

export function createQuestSchedule(
