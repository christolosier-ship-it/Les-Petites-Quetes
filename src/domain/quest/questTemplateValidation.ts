...
const INSTRUCTION_WORD_LIMIT: Readonly<Record<AgeBand, number>> = {
  '3-5': 12,
  '6-8': 20,
  '9-10': 30,
};
...
function validateAgeBands(ageBands: readonly AgeBand[]): readonly AgeBand[] {
  assertDomain(
    ...
    'Une quête doit cibler au moins une tranche d’âge.',
    'ageBands',
  );
  assertDomain(
    !hasDuplicates(ageBands),
    'quest.age-band-duplicated',
    'Une tranche d’âge ne peut apparaître qu’une fois.',
    'ageBands',
  );
  for (const ageBand of ageBands) {
    assertDomain(
      belongsTo(ageBand, AGE_BANDS),
      'quest.invalid-age-band',
      'Une tranche d’âge de la quête est inconnue.',
      'ageBands',
    );
  }
  return [...ageBands];
...
function validateSteps(
  ...
  assertDomain(
    ...
    `Cette quête ne peut pas dépasser ${maximum} étapes pour les âges sélectionnés.`,
    'steps',
  );
  ...
  const normalized = steps.map((step, index) => ({
    id: requireIdentifier(
      ...
      'Chaque étape doit posséder un identifiant.',
      `steps.${index}.id`,
    ),
    instruction: requireText(
      step.instruction,
      'quest.step-instruction-required',
      'Chaque étape doit décrire une action.',
      `steps.${index}.instruction`,
    ),
  }));
  assertDomain(
    !hasDuplicates(normalized.map((step) => step.id)),
    'quest.step-id-duplicated',
    'Deux étapes ne peuvent pas partager le même identifiant.',
    'steps',
  );
  ...
  for (const [index, step] of normalized.entries()) {
    assertDomain(
      wordCount(step.instruction) <= instructionLimit,
      'quest.step-instruction-too-long',
      `Une étape ne peut pas dépasser ${instructionLimit} mots pour les âges sélectionnés.`,
      `steps.${index}.instruction`,
    );
  }
...
export function normalizeQuestTemplateInput(input: QuestTemplateInput): QuestTemplateInput {
  ...
  const title = requireText(
    ...
    'Un titre de quête est requis.',
    'title',
  );
  const instruction = requireText(
    input.instruction,
    'quest.instruction-required',
    'Une consigne enfant est requise.',
    'instruction',
  );
  ...
  assertDomain(
    wordCount(title) <= titleLimit,
    'quest.title-too-long',
    `Le titre ne peut pas dépasser ${titleLimit} mots pour les âges sélectionnés.`,
    'title',
  );
  assertDomain(
    wordCount(instruction) <= instructionLimit,
    'quest.instruction-too-long',
    `La consigne ne peut pas dépasser ${instructionLimit} mots pour les âges sélectionnés.`,
    'instruction',
  );
  assertDomain(
    belongsTo(input.categoryId, QUEST_CATEGORIES),
    'quest.invalid-category',
    'La catégorie de quête est inconnue.',
    'categoryId',
  );
  assertDomain(
    belongsTo(input.readingLevel, READING_LEVELS),
    'quest.invalid-reading-level',
    'Le niveau de lecture de la quête est inconnu.',
    'readingLevel',
  );
  assertDomain(
    belongsTo(input.defaultValidation, VALIDATION_MODES),
    'quest.invalid-validation-mode',
    'Le mode de validation de la quête est inconnu.',
    'defaultValidation',
  );
  ...
  if (input.estimatedMinutes !== undefined) {
    assertDomain(
      Number.isInteger(input.estimatedMinutes) &&
        ...
        input.estimatedMinutes <= MAX_DURATION_MINUTES,
      'quest.invalid-duration',
      `La durée doit être comprise entre 1 et ${MAX_DURATION_MINUTES} minutes.`,
      'estimatedMinutes',
    );
  }
  ...
  if (parentNote !== undefined) {
    assertDomain(
      parentNote.length <= MAX_PARENT_NOTE_LENGTH,
      'quest.parent-note-too-long',
      `La note parent ne peut pas dépasser ${MAX_PARENT_NOTE_LENGTH} caractères.`,
      'parentNote',
    );
  }
  ...
  return {
    ...
    illustrationId: requireIdentifier(
      input.illustrationId,
      'quest.identifier-required',
      'Une illustration est requise.',
      'illustrationId',
    ),
    ageBands,
    ...
    rewardDefinitionId: requireIdentifier(
      ...
      'quest.identifier-required',
      'Une récompense narrative est requise.',
      'rewardDefinitionId',
    ),
