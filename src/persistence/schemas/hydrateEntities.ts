import { createChildProfile, type ChildProfile } from '../../domain/child/ChildProfile';
import type { QuestOccurrence, QuestOccurrenceStatus } from '../../domain/completion/QuestOccurrenceTypes';
...
import { createQuestSchedule, type QuestSchedule, type QuestScheduleInput } from '../../domain/schedule/QuestSchedule';
...
import { WORLD_IDS } from '../../domain/world/WorldDefinition';
import {
  boolean,
  integer,
  list,
...
const OCCURRENCE_STATUSES: readonly QuestOccurrenceStatus[] = ['upcoming', 'available', 'started', 'validation-requested', 'completed', 'postponed', 'ignored'];
const VALIDATION_FEEDBACK: readonly ValidationFeedback[] = ['small-step-remains', 'review-together'];
...
export function hydrateQuestTemplate(value: unknown): QuestTemplate {
  ...
  if (ageBands.some((age) => !AGE_BANDS.includes(age as AgeBand))) throw new Error('ageBands contient une tranche d’âge inconnue.');
  ...
  const input: QuestTemplateInput = {
    ...
    worldId: oneOf(source, 'worldId', WORLD_IDS),
  ...
  const created = templateSource === 'builtin'
    ? createBuiltinQuestTemplate(input, { id: entityMetadata.id, createdAt: entityMetadata.createdAt, contentVersion: text(source, 'contentVersion') })
    : createCustomQuestTemplate(input, { id: entityMetadata.id, createdAt: entityMetadata.createdAt });
  return { ...created, ...entityMetadata, isArchived: boolean(source, 'isArchived') };
}
...
export function hydrateSchedule(value: unknown): QuestSchedule {
  ...
  if (weekdayValues?.some((weekday) => !WEEKDAYS.includes(weekday as Weekday))) throw new Error('weekdays contient un jour inconnu.');
  ...
  const input: QuestScheduleInput = {
    ...
    questFamilyId: text(source, 'questFamilyId'),
    worldId: oneOf(source, 'worldId', WORLD_IDS),
  ...
  const created = createQuestSchedule(input, { id: entityMetadata.id, createdAt: entityMetadata.createdAt });
...
export function hydrateOccurrence(value: unknown): QuestOccurrence {
  ...
  return {
    ...
    ...(postponedToValue !== undefined ? { postponedTo: normalizeLocalDate(postponedToValue, 'postponedTo') } : {}),
...
export function hydrateCompletion(value: unknown): Completion {
  ...
  return { ...metadata(source), occurrenceId: text(source, 'occurrenceId'), childId: text(source, 'childId'), validationMode: oneOf(source, 'validationMode', VALIDATION_MODES), validatedBy: oneOf(source, 'validatedBy', VALIDATORS), completedAt: text(source, 'completedAt'), rewardGrantId: text(source, 'rewardGrantId') };
}

export function hydrateRewardGrant(value: unknown): RewardGrant {
  const source = record(value, 'récompense attribuée');
  return { ...metadata(source), childId: text(source, 'childId'), completionId: text(source, 'completionId'), rewardDefinitionId: text(source, 'rewardDefinitionId'), grantedAt: text(source, 'grantedAt') };
}
