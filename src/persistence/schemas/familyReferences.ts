...
function assertScheduleReferences(state: FamilyState): void {
  ...
  for (const schedule of state.schedules) {
    if (!templateIds.has(schedule.questTemplateId)) {
      throw new Error('Une planification référence une quête inconnue.');
    }
    if (schedule.childIds.some((id) => !childIds.has(id))) {
      throw new Error('Une planification référence un enfant inconnu.');
    }
  }
}
...
function assertOccurrenceReferences(state: FamilyState): void {
  ...
  for (const occurrence of state.occurrences) {
    if (!scheduleIds.has(occurrence.scheduleId)) {
      throw new Error('Une occurrence référence une planification inconnue.');
    }
    if (!templateIds.has(occurrence.questTemplateId)) {
      throw new Error('Une occurrence référence une quête inconnue.');
    }
    if (!childIds.has(occurrence.childId)) {
      throw new Error('Une occurrence référence un enfant inconnu.');
    }
    const key = `${occurrence.scheduleId}:${occurrence.childId}:${occurrence.localDate}`;
    if (occurrenceKeys.has(key)) {
      throw new Error('Deux occurrences partagent la même clé métier.');
    }
...
function assertCompletionReferences(state: FamilyState): void {
  ...
  for (const completion of state.completions) {
    ...
    if (!grantIds.has(completion.rewardGrantId)) {
      throw new Error('Une réalisation référence une récompense absente.');
    }
    if (completedOccurrences.has(completion.occurrenceId)) {
      throw new Error('Une occurrence possède plusieurs réalisations.');
    }
...
function assertRewardReferences(state: FamilyState): void {
  ...
  for (const grant of state.rewardGrants) {
    ...
    if (!rewardIds.has(grant.rewardDefinitionId)) {
      throw new Error('Une récompense attribuée utilise une définition inconnue.');
    }
    if (rewardedCompletions.has(grant.completionId)) {
      throw new Error('Une réalisation possède plusieurs récompenses principales.');
    }
