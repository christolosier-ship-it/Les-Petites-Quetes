...
function hydrateSettings(value: unknown): AppSettings {
  ...
  if (typeof parentPin !== 'string' || !/^$|^\d{4}$/.test(parentPin)) {
  ...
  return {
    ...
    contentVersion: typeof source.contentVersion === 'string'
...
function hydrateState(value: unknown): FamilyState {
  ...
  return {
    ...
    customQuestTemplates: list(source.customQuestTemplates, 'customQuestTemplates')
      .map(hydrateQuestTemplate),
...
function assertFamilySettings(state: FamilyState): void {
  ...
  if (state.settings.activeChildId !== undefined) {
    ...
    if (active === undefined || active.isArchived || active.deletedAt !== undefined) {
      throw new Error('Le profil actif est absent ou archivé.');
    }
  }
  if (
    state.settings.onboardingCompleted &&
    (state.settings.parentPin === '' || state.children.length === 0)
  ) {
