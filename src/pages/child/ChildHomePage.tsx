...
export function ChildHomePage({ app, onBack }: ChildHomePageProps) {
  ...
  const activeChildId =
    children.find((child) => child.id === app.state.settings.activeChildId)?.id ?? children[0]?.id;

  if (activeChildId === undefined) {
    return (
      <section className="centered-panel" data-page="child">
        <Card className="empty-card"><div aria-hidden="true">🌱</div><h2>La forêt attend son explorateur</h2><p>Un adulte doit d’abord créer un profil dans l’espace parent.</p><Button onClick={onBack}>Revenir à l’accueil</Button></Card>
      </section>
    );
  }
  ...
  return (
    <section className="workspace workspace--child" data-page="child">
      ...
      {children.length > 1 && (
        <div className="profile-picker" aria-label="Choisir un profil">
          {children.map((child) => <button key={child.id} type="button" className={child.id === activeChildId ? 'profile-chip profile-chip--active' : 'profile-chip'} onClick={() => void app.selectChild(child.id)}>✨ {child.displayName}</button>)}
        </div>
      )}
