...
export function ParentHomePage({ app, onBack }: ParentHomePageProps) {
  ...
  if (!app.parentUnlocked) {
    return (
      <ParentLock
        hasPin={app.state.settings.parentPin.length > 0}
        onSetPin={app.setParentPin}
        onUnlock={app.unlockParent}
        onBack={onBack}
      />
    );
  }
  ...
  return (
    <section className="workspace" data-page="parent">
      <header className="workspace-header">
        <div><p className="eyebrow">Espace parent</p><h2>Préparer les petites aventures</h2></div>
        <div className="button-row"><Button variant="quiet" onClick={app.lockParent}>Verrouiller</Button><Button variant="quiet" onClick={onBack}>Accueil</Button></div>
      </header>
