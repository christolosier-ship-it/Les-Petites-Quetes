...
function finalizeCompletion(
  ...
  const existing = state.completions.find(
    (completion) => completion.occurrenceId === occurrence.id && completion.deletedAt === undefined,
  );
  if (existing) {
    return replaceOccurrence(state, {
      ...occurrence,
  ...
  return {
    ...
    rewardGrants: grants,
    worldProgress: [
      ...state.worldProgress.filter((candidate) => candidate.id !== progress.id),
      progress,
    ],
  };
}
...
export function finishOccurrence(
  ...
  return requested.status === 'completed'
    ? finalizeCompletion(next, requested, mode, 'child', now, ids)
    : next;
}
...
export function approveOccurrence(
  ...
  return finalizeCompletion(
    replaceOccurrence(state, completed),
    completed,
    mode,
    adultMode,
    now,
...
export function askAnotherStep(
  state: FamilyState,
  occurrenceId: string,
  now: string,
): FamilyState {
...
export function askJointReview(
  state: FamilyState,
  occurrenceId: string,
  now: string,
): FamilyState {
...
export function postponeOccurrence(
  ...
  return replaceOccurrence(
    state,
    postponeQuestOccurrence(occurrenceById(state, occurrenceId), postponedTo, today, now),
  );
}

export function ignoreOccurrence(
  state: FamilyState,
  occurrenceId: string,
  now: string,
): FamilyState {
