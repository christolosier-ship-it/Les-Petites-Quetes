...
export function resetScenePlacement(element: HTMLElement) {
  ...
  element.removeAttribute('data-scene-composer-locked');
}

export function removeSceneDynamicItems(track: HTMLElement) {
  track.querySelectorAll<HTMLElement>(
    '[data-scene-composer-clone="true"], [data-scene-composer-asset="true"]',
  ).forEach((element) => element.remove());
}

function cleanClone(clone: HTMLElement) {
  clone.style.removeProperty('display');
  clone.removeAttribute('data-scene-composer-selected');
...
function createAsset(instance: SceneAssetInstance) {
  ...
  image.src = assetSrc(instance.file);
  image.alt = instance.label;
  image.draggable = false;
  image.className = 'scene-composer__asset-instance';
  image.dataset.sceneComposerAsset = 'true';
  image.dataset.sceneComposerId = instance.id;
  image.style.width = `${instance.width}px`;
  image.style.height = 'auto';
  applyScenePlacement(image, instance);
  return image;
}

export function renderSceneSnapshot(
  track: HTMLElement,
  originals: Map<string, HTMLElement>,
  snapshot: SceneComposerSnapshot,
) {
  const removed = new Set(snapshot.removed);
  for (const [id, element] of originals) {
    element.style.display = removed.has(id) ? 'none' : '';
    applyScenePlacement(element, snapshot.items[id] ?? DEFAULT_SCENE_PLACEMENT);
  }

  removeSceneDynamicItems(track);
  for (const duplicate of snapshot.duplicates) {
    const source = originals.get(duplicate.sourceId);
    if (!source) continue;
    source.after(createDuplicate(source, duplicate));
  }
  for (const asset of snapshot.assets) track.append(createAsset(asset));
}

export function sceneElementById(track: HTMLElement, id: string) {
  return track.querySelector<HTMLElement>(`[data-scene-composer-id="${CSS.escape(id)}"]`);
}

export function sceneElementIds(track: HTMLElement) {
  return [...track.querySelectorAll<HTMLElement>('[data-scene-composer-id]')]
    .filter((element) => element.style.display !== 'none')
    .map((element) => element.dataset.sceneComposerId)
