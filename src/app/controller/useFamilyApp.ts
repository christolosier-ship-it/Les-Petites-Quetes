...
import { addChildProfile, addCustomQuestTemplate, addQuestSchedule } from '../../application/services/familySetupCommands';
...
export function useFamilyApp(): FamilyAppController {
  ...
  const [queue] = useState(() => new StateCommitQueue(createEmptyFamilyState(), async (_current, next) => repository.save(next), setState));

  const refreshBackups = useCallback(async () => setBackups(await repository.listBackups()), []);

  useEffect(() => {
    let active = true;
    void repository.load().then(async (loaded) => {
      const refreshed = refreshScheduledOccurrences(loaded, clock.todayLocal(), clock.nowIso(), ids);
  ...
  const apply = useCallback(async (transform: (current: FamilyState) => FamilyState) => {
    setError(undefined);
    try { await queue.enqueue(transform); }
    catch (reason) {
  ...
  const completeOnboarding = useCallback(async (input: OnboardingInput) => {
    assertPin(input.pin);
    if (input.suggestedTemplateIds.length < 1 || input.suggestedTemplateIds.length > 3) throw new Error('Choisis entre une et trois premières quêtes.');
    await apply((current) => {
      ...
      next = { ...next, settings: { ...next.settings, parentPin: input.pin, defaultValidationMode: input.defaultValidationMode, onboardingCompleted: true } };
      for (const templateId of input.suggestedTemplateIds) {
        ...
        }, clock.nowIso(), clock.todayLocal(), ids);
      }
      return next;
    });
  ...
  const flow = useCallback(async (command: (current: FamilyState, now: string) => FamilyState) => {
    await apply((current) => command(current, clock.nowIso()));
  }, [apply]);

  const createCustomQuest = useCallback(async (
    template: Omit<QuestTemplateInput, 'familyId'>,
    schedule: Omit<QuestScheduleInput, 'questTemplateId' | 'questFamilyId' | 'worldId'>,
  ) => {
    await apply((current) => {
      const created = addCustomQuestTemplate(current, template, clock.nowIso(), ids);
      const createdTemplate = created.state.customQuestTemplates.find((candidate) => candidate.id === created.questTemplateId)!;
      return addQuestSchedule(created.state, {
        ...schedule,
        questTemplateId: createdTemplate.id,
        questFamilyId: createdTemplate.familyId,
        worldId: createdTemplate.worldId,
      }, clock.nowIso(), clock.todayLocal(), ids);
    });
  }, [apply]);

  const importBackup = useCallback(async (content: string) => {
    const imported = parseFamilyBackup(content).state;
    const now = clock.nowIso();
    await queue.enqueue(() => ({ ...imported, settings: { ...imported.settings, lastBackupAt: now } }), (current, next) => repository.replaceWithBackup(current, next, 'before-import', now));
    await refreshBackups();
  }, [queue, refreshBackups]);
