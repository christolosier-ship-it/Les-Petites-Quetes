...
const stageMessages = [
  'Le sentier attend sa première petite lumière.',
  'Quelques lucioles commencent à montrer le chemin.',
  'Les habitants se retrouvent autour du grand arbre.',
  'La forêt brille et de nouveaux passages restent à découvrir.',
] as const;
    ...
    <div className={compact ? 'world-layout world-layout--compact' : 'world-layout'}>
      <Card className={`world-scene world-scene--stage-${stage}`}>
        ...
        <div className="world-scene__overlay">
          ...
          <p>{child.ageBand === '3-5' ? stageMessages[stage] : `${progress?.completionCount ?? 0} petite(s) quête(s) ont fait grandir ce monde.`}</p>
        </div>
      </Card>
      <div className="world-details">
        <Card as="section">
          <p className="eyebrow">Trésor</p><h3>Objets et habitants</h3>
          {rewards.length === 0 ? <p>La première lumière apparaîtra après une quête terminée.</p> : <div className="reward-grid">{rewards.map((reward) => <div key={reward.id} className="reward-token"><span aria-hidden="true">✦</span><strong>{reward.label}</strong><small>{reward.description}</small></div>)}</div>}
        </Card>
        <Card as="section">
          <p className="eyebrow">Histoire</p><h3>Chapitres découverts</h3>
