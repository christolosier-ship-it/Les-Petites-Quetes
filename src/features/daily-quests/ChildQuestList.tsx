...
export function ChildQuestList({ app, childId }: ChildQuestListProps) {
  ...
  const scheduleFor = (occurrence: QuestOccurrence) =>
    app.state.schedules.find((candidate) => candidate.id === occurrence.scheduleId);
  const occurrences = app.state.occurrences
    .filter((item) => {
      const schedule = scheduleFor(item);
      return item.childId === childId && item.deletedAt === undefined && !schedule?.isSuspended &&
        ['available', 'started', 'validation-requested'].includes(item.status);
    })
    .sort((left, right) => {
  ...
  async function finish(occurrence: QuestOccurrence) {
    ...
    if (schedule?.validationMode !== 'child') {
      setNotice('Ta demande est partie vers ton adulte. Tu peux retourner jouer.');
    }
  }
    ...
    <section className="stack" aria-labelledby="child-quests-title">
      ...
      <div className="child-quest-grid">
        {visible.map((occurrence) => {
          ...
          return (
            <button key={occurrence.id} className={optional ? 'child-quest-card child-quest-card--optional' : 'child-quest-card'} data-occurrence-id={occurrence.id} type="button" onClick={() => openQuest(occurrence.id)}>
              <img src={getAssetUrl(asset.id)} alt={asset.alt} />
              <span>{optional && <em>🌱 À choisir</em>}<strong>{template.title}</strong><small>{occurrence.status === 'validation-requested' ? 'En attente de ton adulte' : template.instruction}</small></span>
            </button>
          );
        })}
      </div>
