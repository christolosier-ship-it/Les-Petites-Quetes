...
export class SequenceIds implements IdGenerator {
  private index = 0;
  next(): string { this.index += 1; return `test-id-${this.index}`; }
}

export function createFamilyWithQuest(validationMode: ValidationMode = 'parent'): { readonly state: FamilyState; readonly ids: SequenceIds } {
  ...
  const created = addCustomQuestTemplate(state, {
    ...
    categoryId: 'autonomy',
    illustrationId: 'quest.firefly-forest.autonomy',
    ageBands: ['3-5'],
    readingLevel: 'visual',
    estimatedMinutes: 5,
